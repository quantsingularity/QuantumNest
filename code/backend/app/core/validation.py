...
class FinancialValidator:
    ...
    @staticmethod
    def validate_amount(amount: Union[str, float, Decimal], min_amount: Decimal = None, 
                       max_amount: Decimal = None, allow_negative: bool = False) -> ValidationResult:
        ...
        if min_amount is not None and amount < min_amount:
            result.add_error('amount', f'Amount must be at least {min_amount}', 'below_minimum')
        
        # Check maximum amount
        if max_amount is not None and amount > max_amount:
            result.add_error('amount', f'Amount cannot exceed {max_amount}', 'above_maximum')
        
        # Check decimal places (max 2 for currency)
        if amount.as_tuple().exponent < -2:
            result.add_error('amount', 'Amount cannot have more than 2 decimal places', 'too_many_decimals')
        
        return result
    
    @staticmethod
    def validate_percentage(percentage: Union[str, float, Decimal], 
                          min_value: Decimal = Decimal('0'), 
                          max_value: Decimal = Decimal('100')) -> ValidationResult:
        ...
        try:
            ...
            result.add_error('percentage', 'Invalid percentage format', 'invalid_format')
            return result
        
        if percentage < min_value:
            result.add_error('percentage', f'Percentage must be at least {min_value}%', 'below_minimum')
        
        if percentage > max_value:
            result.add_error('percentage', f'Percentage cannot exceed {max_value}%', 'above_maximum')
    ...
    def validate_symbol(symbol: str) -> ValidationResult:
        ...
        if len(symbol) < 1 or len(symbol) > 10:
            result.add_error('symbol', 'Symbol must be 1-10 characters long', 'invalid_length')
        
        # Check format (alphanumeric only)
        if not re.match(r'^[A-Z0-9]+$', symbol):
            result.add_error('symbol', 'Symbol can only contain letters and numbers', 'invalid_format')
        
        return result
    
    @staticmethod
    def validate_quantity(quantity: Union[str, float, Decimal], 
                         min_quantity: Decimal = Decimal('0')) -> ValidationResult:
        ...
        if quantity < min_quantity:
            result.add_error('quantity', f'Quantity must be at least {min_quantity}', 'below_minimum')
        
        # Check for reasonable decimal places (max 8 for crypto)
        if quantity.as_tuple().exponent < -8:
            result.add_error('quantity', 'Quantity cannot have more than 8 decimal places', 'too_many_decimals')
        
        return result
    
    @staticmethod
    def validate_portfolio_allocation(allocations: Dict[str, Decimal]) -> ValidationResult:
        """Validate portfolio allocation percentages"""
        result = ValidationResult()
        
        if not allocations:
            result.add_error('allocations', 'At least one allocation is required', 'required')
            return result
        ...
        for asset, allocation in allocations.items():
            if allocation < 0:
                result.add_error(f'allocation_{asset}', 'Allocation cannot be negative', 'negative_allocation')
            if allocation > 100:
                result.add_error(f'allocation_{asset}', 'Allocation cannot exceed 100%', 'excessive_allocation')
        
        # Check total allocation
        if abs(total_allocation - 100) > Decimal('0.01'):  # Allow small rounding errors
            result.add_error('total_allocation', 'Total allocation must equal 100%', 'invalid_total')
...
class UserValidator:
    ...
    def validate_name(name: str, field_name: str = 'name') -> ValidationResult:
        ...
        if not name or not isinstance(name, str):
            result.add_error(field_name, f'{field_name.title()} is required', 'required')
            return result
        ...
        # Check length
        if len(name) < 1:
            result.add_error(field_name, f'{field_name.title()} cannot be empty', 'empty')
        elif len(name) > 50:
            result.add_error(field_name, f'{field_name.title()} is too long (max 50 characters)', 'too_long')
        
        # Check for valid characters (letters, spaces, hyphens, apostrophes)
        if not re.match(r"^[a-zA-Z\s\-']+$", name):
            result.add_error(field_name, f'{field_name.title()} contains invalid characters', 'invalid_characters')
    ...
    def validate_username(username: str) -> ValidationResult:
        ...
        # Check length
        if len(username) < 3:
            result.add_error('username', 'Username must be at least 3 characters long', 'too_short')
        elif len(username) > 30:
            result.add_error('username', 'Username is too long (max 30 characters)', 'too_long')
        
        # Check format (alphanumeric, underscore, hyphen)
        if not re.match(r'^[a-zA-Z0-9_-]+$', username):
            result.add_error('username', 'Username can only contain letters, numbers, underscores, and hyphens', 'invalid_format')
        
        # Check that it doesn't start or end with special characters
        if username.startswith(('_', '-')) or username.endswith(('_', '-')):
            result.add_error('username', 'Username cannot start or end with underscore or hyphen', 'invalid_format')
    ...
    def validate_date_of_birth(dob: Union[str, date, datetime]) -> ValidationResult:
        ...
        if age < 18:
            result.add_error('date_of_birth', 'Must be at least 18 years old', 'too_young')
        elif age > 120:
...
class SecurityValidator:
    ...
    def validate_ip_address(ip: str) -> ValidationResult:
        ...
        if not (re.match(ipv4_pattern, ip) or re.match(ipv6_pattern, ip)):
            result.add_error('ip_address', 'Invalid IP address format', 'invalid_format')
    ...
    def validate_api_key_name(name: str) -> ValidationResult:
        ...
        if len(name) < 3:
            result.add_error('name', 'API key name must be at least 3 characters long', 'too_short')
        elif len(name) > 50:
            result.add_error('name', 'API key name is too long (max 50 characters)', 'too_long')
        
        # Allow letters, numbers, spaces, hyphens, underscores
        if not re.match(r'^[a-zA-Z0-9\s\-_]+$', name):
            result.add_error('name', 'API key name contains invalid characters', 'invalid_characters')
...
class TransactionValidator:
    ...
    @staticmethod
    def validate_transaction_type(transaction_type: str, valid_types: List[str]) -> ValidationResult:
        """Validate transaction type"""
        result = ValidationResult()
        
        if not transaction_type:
            result.add_error('transaction_type', 'Transaction type is required', 'required')
            return result
        
        if transaction_type not in valid_types:
            result.add_error('transaction_type', f'Invalid transaction type. Must be one of: {", ".join(valid_types)}', 'invalid_type')
        
        return result
    
    @staticmethod
    def validate_buy_transaction(quantity: Decimal, price: Decimal, 
                               available_balance: Decimal) -> ValidationResult:
        """Validate buy transaction"""
        result = ValidationResult()
        
        # Validate quantity
        quantity_result = FinancialValidator.validate_quantity(quantity, Decimal('0.00000001'))
        ...
        if quantity_result.is_valid and price_result.is_valid:
            ...
            if total_cost > available_balance:
                result.add_error('balance', 'Insufficient balance for this transaction', 'insufficient_balance')
        
        result.is_valid = len(result.errors) == 0
        return result
    
    @staticmethod
    def validate_sell_transaction(quantity: Decimal, price: Decimal, 
                                available_quantity: Decimal) -> ValidationResult:
        ...
        if quantity_result.is_valid and quantity > available_quantity:
            result.add_error('quantity', 'Insufficient quantity available for sale', 'insufficient_quantity')
...
class ComplianceValidator:
    ...
    @staticmethod
    def validate_large_transaction(amount: Decimal, threshold: Decimal = Decimal('10000')) -> ValidationResult:
        """Validate large transactions that may require additional reporting"""
        result = ValidationResult()
        
        if amount >= threshold:
            # This would trigger additional compliance checks
            logger.info(f"Large transaction detected: {amount}", extra={'compliance': True})
        
        return result
    
    @staticmethod
    def validate_suspicious_pattern(user_id: str, transaction_history: List[Dict]) -> ValidationResult:
        """Validate for suspicious transaction patterns"""
        result = ValidationResult()
        
        # This is a simplified example - real implementation would be more sophisticated
        if len(transaction_history) > 10:  # Many transactions in short time
            rapid_transactions = [t for t in transaction_history if 
                                (datetime.now() - t['timestamp']).total_seconds() < 3600]  # Last hour
            
            if len(rapid_transactions) > 5:
                result.add_error('pattern', 'Suspicious transaction pattern detected', 'suspicious_pattern')
                logger.warning(f"Suspicious pattern detected for user {user_id}", 
...
class CompositeValidator:
    ...
    def validate_user_registration(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        # Validate names
        first_name_result = self.user.validate_name(data.get('first_name'), 'first_name')
        ...
        if data.get('phone_number'):
            phone_result = self.user.validate_phone_number(data.get('phone_number'), data.get('country'))
    ...
    def validate_portfolio_creation(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        if not name:
            ...
            result.add_error('name', 'Portfolio name is too long (max 100 characters)', 'too_long')
        
        # Validate initial allocations if provided
        if data.get('allocations'):
            allocation_result = self.financial.validate_portfolio_allocation(data['allocations'])
    ...
    def validate_trade_order(self, data: Dict[str, Any], user_context: Dict[str, Any]) -> ValidationResult:
        """Validate trade order"""
        result = ValidationResult()
        
        # Validate transaction type
        valid_types = ['buy', 'sell']
        type_result = self.transaction.validate_transaction_type(data.get('transaction_type'), valid_types)
        ...
        # Compliance checks
        amount = Decimal(str(data.get('quantity', 0))) * Decimal(str(data.get('price', 0)))
