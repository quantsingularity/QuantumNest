...
class PCAAnalyzer:
    ...
    def fit(self, data: Any, features: Any = None) -> Any:
        """
        ...
        if self.config["n_components"] == "auto":
            ...
            n_components = (
                np.argmax(
                    cumulative_variance >= self.config["explained_variance_threshold"]
                )
                + 1
            )
    ...
    def fit_transform(self, data: Any, features: Any = None) -> Any:
        """
        ...
        importance = pd.DataFrame(
            {
                ...
                "importance": np.average(
                    abs_loadings.values, axis=1, weights=self.explained_variance_ratio_
                ),
            }
        ).sort_values("importance", ascending=False)
        return importance

    def get_explained_variance(self) -> Any:
        """
    ...
    def plot_explained_variance(self, figsize: Any = (10, 6)) -> Any:
        """
    ...
    def plot_feature_importance(
        self, n_features: Any = 10, figsize: Any = (12, 8)
    ) -> Any:
        """
    ...
    def plot_biplot(
        self, transformed_data: Any, features_to_show: Any = 5, figsize: Any = (12, 10)
    ) -> Any:
        """
        ...
        fig, ax = plt.subplots(figsize=figsize)
        ax.scatter(transformed_data["PC1"], transformed_data["PC2"], alpha=0.7, s=30)
        ...
        ax.add_patch(circle)
        ax.set_xlabel(
            f"PC1 ({self.explained_variance_ratio_[0]:.2%} explained variance)"
        )
        ax.set_ylabel(
            f"PC2 ({self.explained_variance_ratio_[1]:.2%} explained variance)"
        )
    ...
    def load(cls: Any, path: Any) -> Any:
        ...
        instance.explained_variance_ratio_ = (
            np.array(data["explained_variance_ratio"])
            if data["explained_variance_ratio"] is not None
            else None
        )
...
if __name__ == "__main__":
    ...
    pca_analyzer = PCAAnalyzer(
        {"n_components": "auto", "explained_variance_threshold": 0.8}
    )
