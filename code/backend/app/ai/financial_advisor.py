...
from app.ai.portfolio_optimization import AdvancedPortfolioOptimizer, OptimizationConstraints
...
class AIFinancialAdvisor:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.config = {
            ...
            'allocation_models': ['target_date', 'risk_parity', 'factor_based', 'goal_based'],
            ...
            # Risk assessment
            'risk_factors': ['age', 'income_stability', 'debt_ratio', 'investment_experience'],
            ...
            # Behavioral factors
            'behavioral_biases': ['loss_aversion', 'overconfidence', 'herding', 'anchoring']
        }
    ...
    async def create_user_profile(self, user_data: Dict[str, Any]) -> UserProfile:
        ...
        try:
            ...
            profile = UserProfile(
                ...
                risk_tolerance=RiskTolerance(user_data.get('risk_tolerance', 'moderate')),
                investment_goals=[InvestmentGoal(goal) for goal in user_data.get('investment_goals', ['wealth_building'])],
                investment_horizon=InvestmentHorizon(user_data.get('investment_horizon', 'long_term')),
                current_portfolio=user_data.get('current_portfolio', {}),
                monthly_investment_capacity=user_data.get('monthly_investment_capacity', 500),
    ...
    async def generate_comprehensive_advice(self, user_id: str) -> List[FinancialAdvice]:
        ...
        try:
            ...
            logger.info(f"Generated {len(advice_list)} recommendations for user {user_id}")
            return advice_list
            
        except Exception as e:
            logger.error(f"Error generating comprehensive advice: {str(e)}", exc_info=True)
            return []
    
    async def generate_portfolio_recommendation(self, user_id: str) -> PortfolioRecommendation:
        ...
        try:
            ...
            recommended_allocation = await self._calculate_target_allocation(profile, allocation_strategy)
            
            # Get specific asset recommendations
            asset_recommendations = await self._recommend_specific_assets(profile, recommended_allocation)
            
            # Calculate expected returns and risk
            expected_return, expected_volatility = await self._calculate_portfolio_metrics(
                recommended_allocation, asset_recommendations
            )
            
            # Generate rationale using AI
            rationale = await self._generate_allocation_rationale(profile, recommended_allocation)
            ...
            tax_implications = self._assess_tax_implications(profile, recommended_allocation)
            ...
            logger.error(f"Error generating portfolio recommendation: {str(e)}", exc_info=True)
            raise
    ...
    async def provide_market_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            insights = {
                ...
                'personalized_opportunities': await self._identify_opportunities(profile, market_data),
    ...
    async def _assess_emergency_fund(self, profile: UserProfile) -> Optional[FinancialAdvice]:
    ...
    async def _assess_debt_management(self, profile: UserProfile) -> Optional[FinancialAdvice]:
        ...
        try:
            ...
            if debt_to_income_ratio > 0.4:  # High debt ratio
                return FinancialAdvice(
                    ...
                    monitoring_metrics=['Debt balance', 'Debt-to-income ratio', 'Credit score'],
                    timestamp=datetime.utcnow()
                )
    ...
    async def _generate_investment_advice(self, profile: UserProfile) -> Optional[FinancialAdvice]:
        ...
        try:
            ...
            if investment_readiness < 0.6:
                return FinancialAdvice(
                    ...
                    supporting_data={'investment_readiness_score': investment_readiness},
                    implementation_steps=[
                    ...
                    monitoring_metrics=['Emergency fund', 'Debt levels', 'Investment balance'],
                    timestamp=datetime.utcnow()
                )
    ...
    async def _generate_tax_advice(self, profile: UserProfile) -> Optional[FinancialAdvice]:
        ...
        try:
            ...
            if profile.income > 50000:
                ...
                tax_strategies.append(f'Maximize 401(k) contribution: ${recommended_contribution:,.0f}')
            ...
            if tax_strategies:
                return FinancialAdvice(
                    ...
                    monitoring_metrics=['Tax savings', 'Retirement contributions', 'Effective tax rate'],
                    timestamp=datetime.utcnow()
                )
    ...
    async def _generate_goal_specific_advice(self, profile: UserProfile, goal: InvestmentGoal) -> Optional[FinancialAdvice]:
    ...
    async def _generate_retirement_advice(self, profile: UserProfile) -> FinancialAdvice:
        ...
        return FinancialAdvice(
            ...
            monitoring_metrics=['Retirement balance', 'Savings rate', 'Investment returns'],
            timestamp=datetime.utcnow()
        )
    
    def _determine_allocation_strategy(self, profile: UserProfile) -> str:
        """Determine appropriate asset allocation strategy"""
        if profile.age < 30 and profile.risk_tolerance in [RiskTolerance.AGGRESSIVE, RiskTolerance.VERY_AGGRESSIVE]:
    ...
    async def _calculate_target_allocation(self, profile: UserProfile, strategy: str) -> Dict[str, float]:
    ...
    async def _recommend_specific_assets(self, profile: UserProfile, allocation: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        asset_mapping = {
            ...
            'international_stocks': [
                {'symbol': 'VXUS', 'name': 'Total International Stock ETF', 'expense_ratio': 0.08},
                {'symbol': 'VTIAX', 'name': 'Total International Stock Index', 'expense_ratio': 0.11}
            ],
            'bonds': [
                {'symbol': 'BND', 'name': 'Total Bond Market ETF', 'expense_ratio': 0.03},
                {'symbol': 'AGG', 'name': 'Core Aggregate Bond ETF', 'expense_ratio': 0.03}
            ],
            'reits': [
    ...
    async def _generate_allocation_rationale(self, profile: UserProfile, allocation: Dict[str, float]) -> str:
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a professional financial advisor providing clear, educational explanations for investment recommendations."},
    ...
    async def _determine_investment_strategy(self, profile: UserProfile) -> Dict[str, Any]:
        ...
        if profile.risk_tolerance == RiskTolerance.AGGRESSIVE:
            return {
                ...
                'monitoring_metrics': ['Portfolio value', 'Asset allocation', 'Rebalancing needs']
            }
        else:
            return {
                'name': 'Balanced',
                ...
                'monitoring_metrics': ['Portfolio balance', 'Risk level', 'Performance vs benchmark']
            }
    
    def _calculate_monthly_savings(self, future_value: float, years: int, annual_return: float) -> float:
    ...
    async def _calculate_portfolio_metrics(self, allocation: Dict[str, float], 
                                         assets: List[Dict[str, Any]]) -> Tuple[float, float]:
        ...
        try:
            ...
            portfolio_return = sum(allocation.get(asset_class, 0) * expected_returns.get(asset_class, 0.06) 
                                 for asset_class in allocation.keys())
            
            # Simplified volatility calculation (assumes some correlation)
            portfolio_volatility = sum(allocation.get(asset_class, 0) * volatilities.get(asset_class, 0.10) 
                                     for asset_class in allocation.keys()) * 0.8  # Correlation adjustment
    ...
    def _calculate_implementation_cost(self, current_portfolio: Dict[str, float], 
        ...
        total_changes = sum(abs(target_allocation.get(asset, 0) - current_portfolio.get(asset, 0)) 
                           for asset in set(list(current_portfolio.keys()) + list(target_allocation.keys())))
        
        return total_changes * 0.001  # 0.1% transaction cost
    
    def _assess_tax_implications(self, profile: UserProfile, allocation: Dict[str, float]) -> Dict[str, Any]:
        """Assess tax implications of allocation"""
        return {
            'tax_efficiency': 'High' if 'bonds' in allocation and allocation['bonds'] < 0.3 else 'Medium',
            'recommended_accounts': {
                'taxable': ['stocks', 'international_stocks'],
                'tax_deferred': ['bonds'],
                'tax_free': ['reits']
            },
            'estimated_tax_drag': allocation.get('bonds', 0) * 0.02  # Simplified calculation
        }
    ...
    async def _generate_market_outlook(self, market_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                model=self.config['openai_model'],
                messages=[
                    {"role": "system", "content": "You are a financial analyst providing concise market insights."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=200,
                temperature=0.5
    ...
    async def _analyze_sectors(self, profile: UserProfile) -> Dict[str, Any]:
        ...
        try:
            ...
            if sector_data:
                top_performers = sorted(sector_data, key=lambda x: x.get('change_percent', 0), reverse=True)[:3]
                underperformers = sorted(sector_data, key=lambda x: x.get('change_percent', 0))[:3]
    ...
    async def _generate_ai_market_insights(self, profile: UserProfile, market_data: Dict[str, Any]) -> str:
    ...
    async def _identify_opportunities(self, profile: UserProfile, market_data: Dict[str, Any]) -> List[str]:
        """Identify investment opportunities"""
        opportunities = []
        
        # Example opportunity identification
        if profile.age < 40 and profile.risk_tolerance in [RiskTolerance.MODERATE, RiskTolerance.AGGRESSIVE]:
    ...
    async def _identify_risks(self, profile: UserProfile, market_data: Dict[str, Any]) -> List[str]:
        """Identify potential risks"""
        risks = []
        
        # Example risk identification
        if profile.current_portfolio:
            concentration_risk = max(profile.current_portfolio.values()) if profile.current_portfolio.values() else 0
            if concentration_risk > 0.3:
                risks.append('High concentration in single asset - consider diversification')
        
        if profile.emergency_fund_months < 3:
            risks.append('Insufficient emergency fund may force early investment liquidation')
        
        return risks
    
    async def _assess_risk_management(self, profile: UserProfile) -> Optional[FinancialAdvice]:
        ...
        try:
            ...
            if risk_factors:
                return FinancialAdvice(
                    ...
                    monitoring_metrics=['Coverage amounts', 'Premium costs', 'Policy terms'],
                    timestamp=datetime.utcnow()
                )
    ...
    async def _generate_education_advice(self, profile: UserProfile) -> FinancialAdvice:
        ...
        years_to_education = 18 - min([child_age for child_age in [10, 15]] if profile.dependents > 0 else [18])  # Assume youngest child is 10
        education_cost = 100000  # Estimated college cost
        
        monthly_savings = self._calculate_monthly_savings(education_cost, years_to_education, 0.06)
        ...
        return FinancialAdvice(
            ...
            monitoring_metrics=['529 balance', 'Investment performance', 'Education cost inflation'],
            timestamp=datetime.utcnow()
        )
    
    async def _generate_home_purchase_advice(self, profile: UserProfile) -> FinancialAdvice:
    ...
    async def _generate_general_goal_advice(self, profile: UserProfile, goal: InvestmentGoal) -> FinancialAdvice:
        ...
        return FinancialAdvice(
            ...
            monitoring_metrics=['Progress toward goal', 'Investment performance', 'Timeline adherence'],
            timestamp=datetime.utcnow()
        )
