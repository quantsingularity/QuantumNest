...
class FinancialAnomalyDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.config = {
            # Detection methods
            'methods': ['isolation_forest', 'autoencoder', 'statistical', 'lstm_autoencoder'],
    ...
    def fit(self, data: pd.DataFrame, target_column: str = 'close') -> Dict[str, Any]:
        ...
        try:
            ...
            if 'isolation_forest' in self.config['methods']:
                results['isolation_forest'] = self._train_isolation_forest(features_standard)
            ...
            if 'lstm_autoencoder' in self.config['methods']:
                results['lstm_autoencoder'] = self._train_lstm_autoencoder(data, target_column)
            ...
            if 'statistical' in self.config['methods']:
                results['statistical'] = {'success': True, 'message': 'Statistical method ready'}
            ...
            logger.error(f"Error training anomaly detection models: {str(e)}", exc_info=True)
            return {'success': False, 'error': str(e)}
    
    def detect_anomalies(self, data: pd.DataFrame, target_column: str = 'close') -> List[AnomalyResult]:
        ...
        try:
            ...
            if 'isolation_forest' in self.config['methods'] and 'isolation_forest' in self.models:
                method_results['isolation_forest'] = self._detect_isolation_forest(features_standard)
            
            if 'autoencoder' in self.config['methods'] and 'autoencoder' in self.models:
                method_results['autoencoder'] = self._detect_autoencoder(features_standard)
            
            if 'lstm_autoencoder' in self.config['methods'] and 'lstm_autoencoder' in self.models:
                method_results['lstm_autoencoder'] = self._detect_lstm_autoencoder(data, target_column)
            
            if 'one_class_svm' in self.config['methods'] and 'one_class_svm' in self.models:
                method_results['one_class_svm'] = self._detect_one_class_svm(features_robust)
            
            if 'statistical' in self.config['methods']:
                method_results['statistical'] = self._detect_statistical(data, target_column)
            
            # Combine results using ensemble method
            anomalies = self._combine_results(method_results, data, target_column)
            
            logger.info(f"Detected {len([a for a in anomalies if a.is_anomaly])} anomalies")
    ...
    def _prepare_features(self, data: pd.DataFrame, target_column: str) -> pd.DataFrame:
        ...
        features['z_score'] = (features['price'] - features['price'].rolling(20).mean()) / features['price'].rolling(20).std()
    ...
    def _train_autoencoder(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            threshold = np.percentile(reconstruction_errors, self.config['autoencoder_threshold'] * 100)
    ...
    def _train_lstm_autoencoder(self, data: pd.DataFrame, target_column: str) -> Dict[str, Any]:
        """Train LSTM autoencoder for sequence anomalies"""
        try:
            # Prepare sequences
            sequences = self._create_sequences(data[target_column].values, self.config['lstm_sequence_length'])
            ...
            model = Sequential([
                LSTM(encoding_dim * 2, activation='relu', input_shape=(input_dim, 1), return_sequences=True),
            ...
            # Reshape for LSTM
            X = sequences_scaled.reshape(sequences_scaled.shape[0], sequences_scaled.shape[1], 1)
            y = sequences_scaled
    ...
    def _train_one_class_svm(self, features: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            model = OneClassSVM(
                kernel='rbf',
                gamma='scale',
                nu=self.config['isolation_contamination']
            )
    ...
    def _detect_isolation_forest(self, features: np.ndarray) -> np.ndarray:
        ...
        scores = (anomaly_scores.max() - anomaly_scores) / (anomaly_scores.max() - anomaly_scores.min())
        return scores
    ...
    def _detect_lstm_autoencoder(self, data: pd.DataFrame, target_column: str) -> np.ndarray:
        ...
        # Scale sequences
        sequences_scaled = self.scalers['lstm'].transform(sequences.reshape(-1, 1)).reshape(sequences.shape)
        X = sequences_scaled.reshape(sequences_scaled.shape[0], sequences_scaled.shape[1], 1)
        
        # Get predictions
        predictions = model.predict(X, verbose=0)
        reconstruction_errors = np.mean(np.square(sequences_scaled - predictions), axis=1)
        
        # Pad scores to match original data length
        scores = np.zeros(len(data))
        start_idx = self.config['lstm_sequence_length']
        scores[start_idx:start_idx + len(reconstruction_errors)] = reconstruction_errors / threshold
    ...
    def _detect_one_class_svm(self, features: np.ndarray) -> np.ndarray:
        ...
        # Normalize scores
        scores = (decision_scores.max() - decision_scores) / (decision_scores.max() - decision_scores.min())
        return scores
    ...
    def _detect_statistical(self, data: pd.DataFrame, target_column: str) -> np.ndarray:
        ...
        z_scores_normalized = np.clip(z_scores / self.config['statistical_threshold'], 0, 1)
    ...
    def _combine_results(self, method_results: Dict[str, np.ndarray], 
                        data: pd.DataFrame, target_column: str) -> List[AnomalyResult]:
        ...
        if self.config['ensemble_method'] == 'voting':
            ...
            weights = {'isolation_forest': 0.3, 'autoencoder': 0.3, 'statistical': 0.2, 
                      'lstm_autoencoder': 0.15, 'one_class_svm': 0.05}
            ...
            valid_results = [scores for scores in method_results.values() if len(scores) == n_points]
            if valid_results:
        ...
        for i, (timestamp, row) in enumerate(data.iterrows()):
            ...
            explanation = self._generate_explanation(i, data, target_column, method_results, score)
    ...
    def _classify_anomaly_type(self, index: int, data: pd.DataFrame, 
                              target_column: str, score: float) -> AnomalyType:
    ...
    def _generate_explanation(self, index: int, data: pd.DataFrame, target_column: str,
                            method_results: Dict[str, np.ndarray], score: float) -> str:
        ...
        if index > 20:
            ...
            if deviation > 10:
                explanations.append(f"Value deviates {deviation:.1f}% from recent average")
        
        return "; ".join(explanations) if explanations else "Anomalous pattern detected"
    
    def _get_context(self, index: int, data: pd.DataFrame, target_column: str) -> Dict[str, Any]:
        ...
        if 'volume' in data.columns:
            ...
            if index > 10:
                context['avg_volume'] = float(data.iloc[index-10:index+1]['volume'].mean())
        ...
        if hasattr(data.index[index], 'dayofweek'):
            ...
            context['hour'] = int(data.index[index].hour) if hasattr(data.index[index], 'hour') else None
    ...
    def save_models(self, filepath: str) -> bool:
        ...
        try:
            ...
            if 'lstm_autoencoder' in self.models:
                self.models['lstm_autoencoder']['model'].save(f"{filepath}_lstm_autoencoder.h5")
                joblib.dump(
    ...
    def load_models(self, filepath: str) -> bool:
        ...
        try:
            ...
            try:
                lstm_autoencoder = tf.keras.models.load_model(f"{filepath}_lstm_autoencoder.h5")
    ...
    def get_model_performance(self, data: pd.DataFrame, target_column: str, 
                            true_anomalies: Optional[List[int]] = None) -> Dict[str, Any]:
