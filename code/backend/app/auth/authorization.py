...
class RoleBasedAccessControl:
    ...
    def __init__(self, db: Session):
        ...
        self.default_roles = {
            ...
            'portfolio_manager': {
                ...
                'permissions': [
                    'portfolio:read', 'portfolio:update', 'portfolio:create',
                    'transaction:read', 'transaction:create',
                    'market_data:read', 'report:read', 'report:create'
                ]
            },
            'trader': {
                ...
                'permissions': [
                    'transaction:read', 'transaction:create',
                    'market_data:read', 'portfolio:read',
                    'account:read'
                ]
            },
            'analyst': {
                ...
                'permissions': [
                    'market_data:read', 'report:read', 'report:create',
                    'portfolio:read', 'transaction:read'
                ]
            },
            'client': {
                'description': 'Client access to own data',
                'permissions': [
                    'account:read', 'portfolio:read', 'transaction:read',
                    'report:read'
                ]
            },
            'compliance_officer': {
                ...
                'permissions': [
                    'transaction:read', 'account:read', 'portfolio:read',
                    'report:read', 'report:create', 'user:read'
                ]
            }
        }
    ...
    def authorize(self, request: AccessRequest) -> AuthorizationResult:
        ...
        try:
            ...
            resource_authorized = self._check_resource_authorization(request, user_permissions)
    ...
    def _check_resource_authorization(self, request: AccessRequest, 
                                    user_permissions: List[str]) -> AuthorizationResult:
        ...
        try:
            ...
            if request.resource == ResourceType.ACCOUNT:
                if request.resource_id:
                    ...
                    if not self._user_owns_resource(request.user_id, 'account', request.resource_id):
                        # Check if user has admin privileges
                        if 'admin:read' not in user_permissions:
                            return AuthorizationResult(
                                granted=False,
                                reason="Access denied: Can only access own accounts",
                                required_permissions=[f"{request.resource.value}:{request.action.value}"],
                                user_permissions=user_permissions,
                                conditions_met=False,
                                risk_level="medium"
                            )
            ...
            # Portfolio access - similar to accounts
                ...
                if request.resource_id:
                    if not self._user_owns_resource(request.user_id, 'portfolio', request.resource_id):
                        if 'admin:read' not in user_permissions and 'portfolio_manager' not in self._get_user_roles(request.user_id):
                            return AuthorizationResult(
                                granted=False,
    ...
    def _check_conditions(self, request: AccessRequest, user_permissions: List[str]) -> bool:
        ...
        try:
            ...
            if request.resource == ResourceType.TRANSACTION and 'amount' in request.context:
...
class AttributeBasedAccessControl:
    ...
    def evaluate_policy(self, subject_attributes: Dict[str, Any],
