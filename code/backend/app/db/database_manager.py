...
class DatabaseManager:
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info("Database manager initialized successfully", 
                       extra={'database_url': self.database_url.split('@')[-1]})  # Hide credentials
    ...
    def _setup_event_listeners(self):
        ...
        def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            ...
            performance_logger.log_database_query(statement.split()[0] if statement else 'UNKNOWN', total_time)
    ...
    def reset_stats(self):
        ...
        with self._lock:
            self._connection_stats.update({
                'total_connections': 0,
                'active_connections': self._connection_stats['active_connections'],  # Keep current active
                'failed_connections': 0,
                'query_count': 0,
                'slow_queries': 0,
                'last_reset': datetime.utcnow()
    ...
    def explain_query(session: Session, query) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(query, 'statement'):
                stmt = str(query.statement.compile(compile_kwargs={"literal_binds": True}))
