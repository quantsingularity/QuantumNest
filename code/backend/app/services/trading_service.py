...
from app.models.models import (
    ...
    TransactionType, TransactionStatus, AssetPrice
)
...
class TradingService:
    ...
    async def place_order(self, order: OrderRequest) -> OrderResult:
        ...
        try:
            ...
            risk_check = await self.risk_manager.evaluate_order_risk(order, current_price)
            ...
            return OrderResult(
                success=False,
                message="Internal error occurred while placing order"
            )
    ...
    async def _validate_order(self, order: OrderRequest) -> OrderResult:
        ...
        portfolio = self.db.query(Portfolio).filter(
            ...
            Portfolio.owner_id == order.user_id
        ).first()
        if not portfolio:
        ...
        if not asset or not asset.is_tradable:
            return OrderResult(success=False, message=f"Asset {order.symbol} is not tradable")
    ...
    async def _execute_market_order(self, order: OrderRequest, current_price: Decimal) -> OrderResult:
        ...
        try:
            ...
            if order.side == OrderSide.BUY:
                ...
                if not await self._check_sufficient_balance(order.user_id, required_balance):
                    return OrderResult(
                        success=False,
                ...
                if not await self._check_sufficient_quantity(
                    ...
                    return OrderResult(
                        success=False,
                        message="Insufficient quantity for sale"
                    )
            ...
            transaction = Transaction(
                ...
                asset_id=self.db.query(Asset).filter(Asset.symbol == order.symbol).first().id,
            ...
            await self._update_portfolio_position(
                order.portfolio_id, order.symbol, order.side, 
    ...
    async def _execute_limit_order(self, order: OrderRequest, current_price: Decimal) -> OrderResult:
        ...
        try:
            ...
            if can_execute:
                ...
                return OrderResult(
                    success=True,
                    message="Limit order placed and waiting for execution"
                )
            ...
            return OrderResult(
                success=False,
                message="Failed to execute limit order"
            )
    
    async def _check_sufficient_balance(self, user_id: int, required_amount: Decimal) -> bool:
        """Check if user has sufficient balance for purchase"""
        # This would check user's cash balance
        # For now, assume sufficient balance
        return True
    
    async def _check_sufficient_quantity(self, portfolio_id: int, symbol: str, required_quantity: Decimal) -> bool:
    ...
    async def _update_portfolio_position(self, portfolio_id: int, symbol: str, 
        ...
        if side == OrderSide.BUY:
            if portfolio_asset:
                ...
                total_cost = (portfolio_asset.quantity * portfolio_asset.average_cost) + (quantity * price)
    ...
    async def get_order_history(self, user_id: int, portfolio_id: Optional[int] = None, 
    ...
    async def get_portfolio_performance(self, portfolio_id: int, 
        ...
        try:
            ...
            portfolio_assets = self.db.query(PortfolioAsset).filter(
                PortfolioAsset.portfolio_id == portfolio_id
            ).all()
            ...
            for pa in portfolio_assets:
                # Get current price
                current_price = await self.market_data.get_current_price(pa.asset.symbol)
                if current_price:
                    ...
                    positions.append({
                        ...
                        'return_pct': float((unrealized_pnl / cost_basis) * 100) if cost_basis > 0 else 0
                    })
            
            total_return = total_value - total_cost
            total_return_pct = float((total_return / total_cost) * 100) if total_cost > 0 else 0
            ...
            self.logger.error(f"Error calculating portfolio performance: {str(e)}", exc_info=True)
...
class AlgorithmicTradingService:
    ...
    async def execute_momentum_strategy(self, portfolio_id: int, symbols: List[str],
                                      lookback_days: int = 20, threshold: float = 0.05) -> Dict:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if momentum > threshold:
                    ...
                    order = OrderRequest(
                        user_id=self.db.query(Portfolio).filter(Portfolio.id == portfolio_id).first().owner_id,
                        portfolio_id=portfolio_id,
                        symbol=symbol,
                        side=OrderSide.BUY,
                        order_type=OrderType.MARKET,
            ...
            self.logger.error(f"Error executing momentum strategy: {str(e)}", exc_info=True)
            return {'error': 'Failed to execute momentum strategy'}
    
    async def execute_mean_reversion_strategy(self, portfolio_id: int, symbols: List[str],
                                            lookback_days: int = 20, std_threshold: float = 2.0) -> Dict:
        ...
        try:
            ...
            self.logger.error(f"Error executing mean reversion strategy: {str(e)}", exc_info=True)
