...
class MarketDataService:
    ...
    async def get_historical_prices(self, symbol: str, days: int = 30, 
        ...
        try:
            ...
            if self._is_cached(cache_key, timeout=300):  # 5 minute cache for historical data
                return self._cache[cache_key]['data']
            ...
            for date, row in hist.iterrows():
                prices.append({
                    ...
                    'adjusted_close': float(row['Close'])  # Yahoo Finance adjusts automatically
                })
        ...
        try:
            ...
            for symbol, result in zip(symbols, results):
                if isinstance(result, MarketQuote):
                    ...
                    self.logger.warning(f"Failed to get quote for {symbol}: {str(result)}")
    ...
    async def get_market_movers(self, market: str = "US", limit: int = 20) -> Dict[str, List[Dict]]:
    ...
    async def get_technical_indicators(self, symbol: str, period: int = 20) -> Dict[str, float]:
