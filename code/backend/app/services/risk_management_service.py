...
from app.models.models import (
    User, Portfolio, PortfolioAsset, Asset, Transaction, 
    RiskLevel, AssetType
)
...
class RiskManagementService:
    ...
    async def calculate_portfolio_risk(self, portfolio_id: int) -> Optional[RiskMetrics]:
        ...
        try:
            ...
            positions = self.db.query(PortfolioAsset).filter(
            ...
            for position in positions:
                current_price = await self.market_data.get_current_price(position.asset.symbol)
                if current_price:
            ...
            var_95, var_99, expected_shortfall = self._calculate_var(returns_data, total_value)
            ...
            self.logger.error(f"Error calculating portfolio risk: {str(e)}", exc_info=True)
            return None
    
    async def evaluate_order_risk(self, order, current_price: Decimal) -> OrderRiskCheck:
        ...
        try:
            ...
            current_positions = self.db.query(PortfolioAsset).filter(
            ...
            if total_portfolio_value > 0:
                ...
                if position_size_ratio > self.max_position_size:
                    risk_score += 0.3
                    warnings.append(f"Large position size: {position_size_ratio:.1%} of portfolio")
            ...
            if asset and asset.sector:
                sector_exposure = self._calculate_sector_exposure(
                    current_positions, asset.sector, order_value if order.side == "buy" else -order_value
                )
                if sector_exposure > self.max_sector_concentration:
                    risk_score += 0.2
                    warnings.append(f"High sector concentration: {sector_exposure:.1%} in {asset.sector}")
            
            # Liquidity check
            if asset:
                liquidity_score = await self._assess_liquidity(asset.symbol, order.quantity)
            ...
            if user and user.risk_tolerance:
                ...
                if not self._is_suitable_for_risk_tolerance(asset_risk, user.risk_tolerance):
                    risk_score += 0.2
    ...
    async def check_risk_limits(self, portfolio_id: int) -> List[RiskLimit]:
        ...
        try:
            ...
            limits.append(RiskLimit(
                ...
                utilization=float((risk_metrics.var_95 / var_limit) * 100) if var_limit > 0 else 0,
            ...
            limits.append(RiskLimit(
                ...
                utilization=float((risk_metrics.volatility / vol_limit) * 100) if vol_limit > 0 else 0,
    ...
    def _calculate_var(self, returns: List[float], portfolio_value: Decimal) -> Tuple[Decimal, Decimal, Decimal]:
        ...
        expected_shortfall = np.mean(tail_losses) * float(portfolio_value) if len(tail_losses) > 0 else 0
        
        return abs(Decimal(str(var_95))), abs(Decimal(str(var_99))), abs(Decimal(str(expected_shortfall)))
    ...
    async def _calculate_beta(self, position_data: List[Dict], returns: List[float]) -> Optional[Decimal]:
        """Calculate portfolio beta against market benchmark"""
        try:
            # Get benchmark returns (using SPY as market proxy)
            benchmark_prices = await self.market_data.get_historical_prices("SPY", days=len(returns))
    ...
    def _calculate_concentration_risk(self, position_data: List[Dict]) -> Dict[str, float]:
        ...
        for pos in position_data:
            ...
            asset_type_weights[asset_type] = asset_type_weights.get(asset_type, 0) + pos['weight']
        
        max_asset_type_weight = max(asset_type_weights.values()) if asset_type_weights else 0
    ...
    async def _calculate_correlation_matrix(self, position_data: List[Dict]) -> Optional[Dict[str, Dict[str, float]]]:
    ...
    async def _get_portfolio_returns(self, position_data: List[Dict]) -> List[float]:
        ...
        try:
            ...
            for pos in position_data:
                prices = await self.market_data.get_historical_prices(pos['symbol'], days=60)
                if len(prices) < 30:
                    continue
    ...
    def _calculate_sector_exposure(self, positions: List, sector: str, additional_value: Decimal = Decimal('0')) -> Decimal:
    ...
    def _is_suitable_for_risk_tolerance(self, asset_risk: RiskLevel, user_risk_tolerance: RiskLevel) -> bool:
    ...
    def _calculate_overall_risk_score(self, risk_metrics: RiskMetrics, risk_limits: List[RiskLimit]) -> float:
    ...
    def _generate_risk_recommendations(self, risk_metrics: RiskMetrics, risk_limits: List[RiskLimit]) -> List[str]:
        ...
        for limit in risk_limits:
            if limit.breached:
                if "Concentration" in limit.name:
                    recommendations.append(f"Consider diversifying to reduce {limit.name.lower()}")
                elif "Volatility" in limit.name:
                    recommendations.append("Consider adding lower volatility assets to reduce portfolio risk")
                elif "Value at Risk" in limit.name:
                    recommendations.append("Consider reducing position sizes or hedging to lower VaR")
        
        # Volatility recommendations
        if risk_metrics.volatility > Decimal('0.3'):
            recommendations.append("High portfolio volatility detected - consider rebalancing with stable assets")
        
        # Concentration recommendations
        for risk_type, concentration in risk_metrics.concentration_risk.items():
            if concentration > 0.25:
                recommendations.append(f"High {risk_type.replace('_', ' ')} concentration - consider diversification")
        ...
        if risk_metrics.sharpe_ratio and risk_metrics.sharpe_ratio < Decimal('0.5'):
            recommendations.append("Low risk-adjusted returns - review asset selection and allocation")
