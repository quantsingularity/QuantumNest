...
def check_code_quality():
    ...
    checks = [
        ("Line length check", "find app -name '*.py' -exec wc -L {} + | awk '$1 > 120 {print $2 \" has lines longer than 120 characters\"}'"),
        ("TODO/FIXME check", "grep -r 'TODO\\|FIXME\\|XXX' app/ || echo 'No TODO/FIXME/XXX found'"),
        ("Print statement check", "grep -r 'print(' app/ | grep -v logger | grep -v '# debug' || echo 'No problematic print statements found'")
    ]
...
def main():
    ...
    parser.add_argument('--quality', action='store_true', help='Check code quality only')
