...
const PORTFOLIO_MANAGER_ADDRESS =
  process.env.NEXT_PUBLIC_PORTFOLIO_MANAGER_ADDRESS || "0x123...";
const TOKENIZED_ASSET_ADDRESS =
  process.env.NEXT_PUBLIC_TOKENIZED_ASSET_ADDRESS || "0x456...";
const DEFI_INTEGRATION_ADDRESS =
  process.env.NEXT_PUBLIC_DEFI_INTEGRATION_ADDRESS || "0x789...";
const TRADING_PLATFORM_ADDRESS =
  process.env.NEXT_PUBLIC_TRADING_PLATFORM_ADDRESS || "0xabc...";
...
class BlockchainService {
  ...
  async getUserPortfolio(): Promise<PortfolioAsset[]> {
    ...
    try {
      const portfolioCount = await this.portfolioManagerContract.methods
        .getUserPortfolioCount(this.account)
        .call();
      const portfolio: PortfolioAsset[] = [];

      for (let i = 0; i < portfolioCount; i++) {
        const asset = await this.portfolioManagerContract.methods
          .getUserPortfolioAsset(this.account, i)
          .call();
        portfolio.push({
          id: asset.id,
          name: asset.name,
          symbol: asset.symbol,
          quantity: parseFloat(
            this.web3!.utils.fromWei(asset.quantity, "ether"),
          ),
  ...
  async buyAsset(
    ...
    try {
      ...
      const tx = await this.tradingPlatformContract.methods
        .buyAsset(symbol, quantityWei)
        .send({
          from: this.account,
          ...options,
        });
  ...
  async stakeTokens(
    ...
    try {
      ...
      const tx = await this.defiIntegrationContract.methods
        .stake(amountWei)
        .send({
          from: this.account,
          ...options,
  ...
  async getStakingRewards(): Promise<number> {
    ...
    try {
      const rewards = await this.defiIntegrationContract.methods
  ...
  async getTransactionHistory(limit: number = 10): Promise<TransactionData[]> {
    ...
    try {
      ...
      for (
        let i = latestBlock;
        i >= fromBlock && transactions.length < limit;
        i--
      ) {
